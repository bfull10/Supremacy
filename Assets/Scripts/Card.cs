...
public class Card : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //If card is currently selected, implement following functionality
        if(isSelected && BattleController.instance.battleEnded == false && Time.timeScale != 0f)
        {
            ...
            //Store info on if we hit anything with our 'ray' in variable 'hit', 100f is 'how far our line should go', then whatIsDesktop is 'what the ray should interact with/hit'
            //Evaluates to true if last parameter, here whatIsDesktop, is being hovered over
            if(Physics.Raycast(ray, out hit, 100f, whatIsDesktop))
            {
                //point is the point where the ray hit against the object
            ...
            if(Input.GetMouseButtonDown(0) && justPressed == false && BattleController.instance.battleEnded == false)
            {
                if(Physics.Raycast(ray, out hit, 100f, whatIsPlacement) && BattleController.instance.currentPhase == BattleController.TurnOrder.playerActive)
                {
                    ...
                    {
                        //Check if player has enough mana to play the card, and if so then play the card
    ...
    {
        if(inHand && BattleController.instance.currentPhase == BattleController.TurnOrder.playerActive && BattleController.instance.battleEnded == false && Time.timeScale != 0f)
        {
            isSelected = true;
    ...
    public void DamageCard(int damageAmount)
    {
        ...
        if(currentHealth <= 0)
        {
            ...
            //Float parameter specifies how long to wait before destroying object
